;(function($)
{
	...
	function setErrorStr (jEle, sErrStr, aParas)
	{
		...
		var sErrMsg = jEle.attr("errMsg") || Utils.Base.sprintf(sErrStr, aParas);
	...
	function checkEle(jEle)
	{
		...
		var validateEle = function(pfCheck, jEle)
		{
			...
			if(null != sErrType)
			{
				...
				var sMsg = $.MyLocale.Validator[sErrType];
		...
		{
			sError =  $.MyLocale.Validator.required; // "This field is required.";
		...
		if(sCtrlName)
		{
			bRight = (jEle.data(sCtrlName)&&jEle.data(sCtrlName)["check"]) ? jEle[sCtrlName]("check") : true;
		}
		else
		{
	...
	function autoCheckForm(jForm)
	{
		...
		$("input:[type='radio'], input:[type='checkbox'], select,.port-list,.m-input", jForm).on("change.form",function(){
			removeDisable(jForm);
		});
		...
		$(".datewidget,.timewidget").bind("changeDate",function(){
           removeDisable(jForm);
        });
		...
		//暂时注释，小贝嵌入式没有
		//$("button.btn-primary", jForm).bind("click",function(){
			//	removeDisable(jForm);
			//});
		//file
	...
	Utils.regUtil(UTILNAME, oWidget, {"widgets": [], "utils":["Error"]});
})(jQuery);