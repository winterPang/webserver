...
(function(jq, undefined) {
    ...
    */
    ...
    CanvasControl.prototype.getFinalCfg = function() {
    	var cfg = canvasControl.canvasUi.getCfg(true);
        return cfg;
    }
    
    // 获取文本广告id
    CanvasControl.prototype.getTextAdIds = function() {
        return canvasControl.canvasUi.getTextAdIds();
    }
    
    // 获取文本广告id
    CanvasControl.prototype.getImagedIds = function() {
        return canvasControl.canvasUi.getImagedIds();
    }
    ...
    CanvasUi.prototype.addMenu = function(){
        ...
        this.canvasMenuText.unbind("click").click(function(event) {
            var wenziMenu = jq('<div id="text'+jq.getRandomDateBetween(new Date())+'" class="adClass textAd canvas-edited-text canvas-event-selectable canvas-event-draggable '
         ...
         this.canvasMenuImg.unbind("click").click(function(event) {
             var imgMenu = jq('<div style="position: relative;" id="image'+jq.getRandomDateBetween(new Date())+'"  class="adClass imageAd canvas-edited-text canvas-event-selectable '
                ...
                showSelected : function() {
                	...
                	toolbar.bind({
    	        	    click : function(event){
    	        	    	...
    	        	    	// 先保存
    	        	    	canvasControl.canvasUi.getCfg(true);
    	        	    	// 自动保存图片的配置
    	        	    	canvasControl.canvasUi.curElm = jqSelf;
    	        	    	...
    	        	    	if("text-role-panel" == jqElm.attr("data-canvasrole")){
    	        	    		...
    	        	    		// 在直接获取
        	        	    	canvasControl.canvasUi.getCfg(false);
    	        	    		jq(this).imgList();
    ...
    // 获取图片配置信息
    CanvasUi.prototype.getCfg = function(flag){
...
//    	}
    	...
    	// 因为很多种情况初始化都会导致找不到属性，所以简单处理tr...catch方式，如果有问题，就获取缺省的配置，否则全部获取最新的配置信息。
    	// flag:如果为true获取最新的配置，否则直接获取之前的配置
    	if(!flag){
    		return canvasControl.opts.jsonCfg;
    	}
        try{
    ...
    // 获取文本广告id,href
	CanvasUi.prototype.getTextAdIds = function() {
		var idHrefArray = [];
 		jq(".textAd").each(function(index, element) {
 			var tempObj = {};
 			// 文本广告id
			var textId = jq(this).attr("id");
			tempObj.textId = textId;
			// 文本链接，-1：空
			var href = "-1";
			if (jq("#" + textId).find("a").length > 0) {
			   href = jq("#" + textId).find("a").attr("href");
			}
			tempObj.href = href;
			// 文本内容 noText：空
			var textContent = "noText"; 
			var adText = jq.trim(jq("#" + textId).find("p").text());
			if (adText != "") {
				textContent = adText;
			}
			tempObj.textContent = textContent;
			idHrefArray.push(tempObj);
		});
		var idHrefString = JSON.stringify(idHrefArray);
		return idHrefString;
	}
	
    // 获取图片广告id,href,url
	CanvasUi.prototype.getImagedIds = function() {
		var imageCfg = JSON.parse(canvasControl.canvasUi.getCfg(true));
		var idHref = "";
		jq.each(imageCfg, function(valueKey, value) {
			jq.each(value, function(valueIndex, adValue) {
			    var starIndex = adValue[0].indexOf("/");
			    var endIndex = adValue[0].indexOf(".");
			    // 截取图片名称用于唯一标识
			    var imageName = adValue[0].substring(starIndex + 1, endIndex);
				...
				// 广告图片url
				var imageUrl = adValue[0];
				idHref += imageId +"_" + href + "_" + imageUrl + "_" + adStarttime + "_" + adEndtime + ",";
			});
		});
		if (null != idHref && "" != idHref) {
			idHref = idHref.substring(0, idHref.length - 1);
		}
		return idHref;
	}

    /*
	 * 所有对所有插件的定义。 但是插件的创建、引用及销毁尽可能由其本身定义及控制。
	 */
    var CanvasWidget = {
		// 文字面板的定义
            		...
            		canvasControl.canvasUi.imgPanel.iContent.append(jq('<div class="canvas-eachItem">' +
            				  ...
            				  '<div class="canvas-imgcontent-right">' +
            				    ...
            				    '<div style="height:50px;margin-left:5px;">' +
            				                       '图片链接：<input type="text" maxlength="128" placeholder="http 或 https开头" class="canvas-text-80 getTxt"/>' +
            				    '</div>' +
            				  '</div>' +
        	...
        	if('' != content && null != content && undefined != content){
        		...
        		for (var i in content) {
        			...
        			parent.iContent.append(jq('<div class="canvas-eachItem">' +
            				  '<div class="canvas-imgcontent-left"><img title="上传图片" id="'+canvasControl.canvasUi.curElm.attr("id")+'_'+jq.getRandomDateBetween(new Date()) +'" class="getImg" src="'+ (undefined == content[i][0] ? "":content[i][0]) + '" width="100px" height="100px"/></div>' +
            				  '<div class="canvas-imgcontent-right">' +
          				    '<div style="height:45px;margin-left:5px;">' +
          				    ...
          				    '</div>' +
          				    '<div style="height:50px;margin-left:5px;">' +
          				                       '图片链接：<input type="text" maxlength="128" class="canvas-text-80 getTxt" placeholder="http 或 https开头"  value="'+(undefined == content[i][3] || "-1" == content[i][3] ? "":content[i][3])+'"/>' +
          				    '</div>' +
          				  '</div>' +
          				  '<div class="canvas-imgdel-link"><a href="#" name="canvasFile"><i class="fa fa-trash-o fa-lg"></i></a></div>' +
          				'</div>'));
