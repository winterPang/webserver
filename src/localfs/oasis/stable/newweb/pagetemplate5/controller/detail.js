define(['utils', 'jquery', 'angular-ui-router', 'bsTable', 'css!pagetemplate5/css/pagetemplate5'], function (Utils, $) {
    return ['$scope', '$http', '$rootScope', '$compile', '$alertService', '$state', '$location', '$window', function ($scope, $http, $rootScope, $compile, $alert, $state, $location, $window) {
        ...
        $.each(['themelist', 'vigourTable'], function (idx, val) {
            $scope.$on('expanded-row.bs.table#' + val, function (e, data) {
        ...
        var tableCommon = {
            ...
            columns: [
                {searcher: {type: 'text'}, sortable: true, field: 'themeName', title: getRcString("Page_HEADER")[0]},
                {searcher: {type: 'text'}, sortable: true, field: 'description', title: getRcString("Page_HEADER")[1]}
            ]
        };
        $scope.pageOption = $.extend(true, {}, tableCommon, {
            tId: 'themelist',
            //获取数据的接口
            url: '/v3/ace/oasis/auth-data/o2oportal/themetemplate/query?ownerName=' + ownerName + '&storeId=' + storeId + '&v3flag=1',
        ...
        $scope.vigourTable = $.extend(true, {}, tableCommon, {
            tId: 'vigourTable',
            url: '/v3/ace/oasis/auth-data/o2oportal/pagetemplate/query?storeId=' + storeId + '&v3flag=1'
        });
        ...
        $scope.refresh = function () {
            var tid;
            if ($scope.flags.temp == 'vigour') {
                tid = 'vigourTable';
            } else if ($scope.flags.temp == 'oldtemp') {
                tid = 'themelist';
            }
            $scope.$broadcast('refresh#' + tid);
        };
            ...
            options: {
                ...
                okHandler: function (modal, $ele) {
                    ...
                    $http({
                        ...
                        $scope.flags.temp = $scope.add.tType == 3 ? 'vigour' : 'oldtemp';
                        if (data.errorcode == 0) {
                            $scope.refresh();
        ...
        //delete
        $scope.del = function (arg) {
            var urlReplace = $scope.flags.temp == 'vigour' ? 'pagetemplate' : $scope.flags.temp == 'oldtemp' ? 'themetemplate' : '';
