;(function ($) {
    ...
    function drawScatterinfor () 
    {
        ...
        function onMapSelect(param){
            Utils.Base.redirect ({np:"WDashboard.device_detail"});
            return false;
        }
        ...
        $.ajax({
            ...
            success:function(data){
                if(data){
                    ...
                    var onlineUser = message[0].statUserNum || 0;
                    $("#onlineUser").text(onlineUser);
                }
            },
            error:function(){
    ...
    //渲染在线AP，在线终端数据
    function initOnlineAp_sta() 
    {
        $.ajax({
            url:MyConfig.path+"/apmonitor/getapandstationcount",
            type: "get",
            dataType: "json",
            contentType: 'application/json',
            success: function(data){
                if(data)
                {
                    var onlineAp =data.count.apcount;
                    var onlineSta =data.count.clientcount;
                    $("#onlineap").text(onlineAp);
                    $("#onlineSta").text(onlineSta);
                }
            },
            error: function(){
    ...
    function getLinkhistoryinfor(obj,sType) {
        ...
        if( g_selectType == "onlineAc")
        {
            //刷新在线AC对应视图
            refreshOnlineAcData(obj,sType);
        }
        else if( g_selectType == "onlineUser")
        {
            //刷新在线租户对应视图
            refreshOnlineUserData(obj,sType);
        }
        else if( g_selectType == "onlineAp")
        {
            //刷新在线AP对应视图
            refreshOnlineApData(obj,sType);
        }
        else if( g_selectType == "onlineClient")
        {
            //刷新在线终端数据
            refreshOnlineClientData(obj,sType);
        }
    }
    ...
    function initForm ()
    {
        ...
        $("#week_history").on("click",function(){
        ...
        $("#moun_history").on("click",function(){
        ...
        /*AC,租户,AP,终端图标点击事件 Start*/
        $("#onlineAC_icon").on("click",function(){
            g_selectType = "onlineAc";
            $("#describe").html("AC在线");
            ...
            //刷新图形在线AC数据，默认为日
            refreshOnlineAcData({Interval:1,Count:24},"Day");
        });

        $("#onlineUser_icon").on("click",function(){
            g_selectType = "onlineUser";
            $("#describe").html("租户在线");
            ...
            //刷新图形在线租户信息，默认为日
            refreshOnlineUserData({Interval:1,Count:24},"Day");
        });

        $("#onlineAP_icon").on("click",function(){
            g_selectType = "onlineAp";
            $("#describe").html("AP在线");
            ...
            //刷新图形在线AP数据，默认为日
            refreshOnlineApData({Interval:1,Count:24},"Day");
        });

        $("#onlineClient_icon").on("click",function(){
            g_selectType = "onlineClient";
            $("#describe").html("终端在线");
            ...
            //刷新图形在线终端数据，默认为日
            refreshOnlineClientData({Interval:1,Count:24},"Day");
        });
        /*End*/
    }

    /*刷新图形在线AC数据*/
    function refreshOnlineAcData(obj,sType){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                /*解析在线AC数据（日,周,月视图）*/
                analyseOnlineAcData(data.reverse());
            },
            error: function(){
    ...
    /*解析在线AC数据*/
    function analyseOnlineAcData(data){
        ...
        for(var i = 0, l = data.length ; i < l ; i++){
            var xdate = onExchangeTime(data[i].recordTime);
            aDateTime.push(xdate);
            aOnlineAc.push(data[i].onlineCount);
        }

        /*draw折线图*/
        drawLine(aDateTime,aOnlineAc);
    }


    //刷新图形在线租户数据
    function refreshOnlineUserData(obj,sType){

        var count = obj.Interval * obj.Count;
        $.ajax({
            url:MyConfig.path + "/v3/scenarioserver",
            type: "post",
            dataType: "json",
            ...
            success:function(data){
                ...
                /*解析在线租户数据*/
                analyseOnlineUserData(obj.Interval,data,sType);
            },
            error: function(){
    ...
    //解析在线租户数据
    function analyseOnlineUserData(interval,data,sType){

        data = data.message.reverse() || [];
        var onlineUserArr = new Array();
        var xlist = new Array();
        ...
        switch(sType)
        {
            case "Day":
            {
                for(var i = 0, l = data.length ; i < l ; i++){
                    ...
                    if( i % interval == 0)
                    {
                        onlineUserArr.push(data[i].statUserNum);
                        if( data[i].statTime < 10){
                            xlist.push("0" + data[i].statTime + ":00");
                        }
                        else
                        {
                            xlist.push(data[i].statTime + ":00");
                        }
                    }
                }
                break;
            ...
            case "Week":
            {
                for(var i = 0, l = data.length ; i < l ; i++){
                    ...
                    {
                        onlineUserArr.push(data[i].statUserNum);
                        xlist.push( gWEEK[new Date(data[i].statDate).getDay()] );
                    }
                }
                break;
            }
        ...
        /*draw折线图*/
        drawLine(xlist,onlineUserArr);
    }

    //刷新在线AP数据
    function refreshOnlineApData(obj,sType){

        g_DateType = sType;
        $.ajax({
            url:MyConfig.path + "/diagnosis_read/web/getApStationHis",
            type:"post",
            dataType:'json',
            contentType: 'application/json',
            data:JSON.stringify({
            ...
            success:function(data){
                ...
                //解析在线AP数据
                analyseOnlineApData(data.reverse());
            },
            error:function(){
    ...
    //解析在线AP数据
    function analyseOnlineApData(data){
        ...
        for(var i = 0, l = data.length ; i < l ; i++){
            var xdate = onExchangeTime(data[i].recordTime);
            aDateTime.push(xdate);
            aOnlineAp.push(data[i].apCount);
        }

        /*draw折线图*/
        drawLine(aDateTime,aOnlineAp);
    }

    //刷新在线终端数据
    function refreshOnlineClientData(obj,sType){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                //解析在线终端数据
                analyseOnlineClientData(data.reverse());
            },
            error:function(){
    ...
    //解析在线终端数据
    function analyseOnlineClientData(data){
        ...
        for(var i = 0, l = data.length ; i < l ; i++){
            var xdate = onExchangeTime(data[i].recordTime);
            aDateTime.push(xdate);
            aOnlineClient.push(data[i].stationCount);
        }

        /*draw折线图*/
        drawLine(aDateTime,aOnlineClient);
    }

    /*draw折线图*/
    function drawLine(xlist,data){
            ...
            series: [
                {
                    ...
                    smooth: true,
                    stack: '总量',
                    itemStyle: {normal: {areaStyle: {color:'#00ccff',type: 'default'},lineStyle:{width:0}}},
                    data:data
                }
            ]
        };
    ...
    function onExchangeTime(sTime){
        ...
        {
            case "Day":
                var currentHour = jDate.getHours();
                if( currentHour < 10){
