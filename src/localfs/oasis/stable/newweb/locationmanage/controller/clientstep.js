define([
    ...
    return ['$scope',
            ...
            '$stateParams',
            'locSer',
            'jtopoSer',function ($scope,$alert,$timeout,$state,$stateParams,locSer,jtopoSer) {
        ...
        $scope.info  = {
            macEmpty:'mac地址不能为空',
            formatError:'mac地址格式不正确！mac地址格式为00:24:21:19:BD:E4',
            startEmpty:'请选择开始时间',
            endEmpty:'请选择结束时间'
        };
        $scope.dragOption = {
        ...
        $scope.toClientMap = function () {
            $state.go('^.clientmap',{shopId:shopId,locationId:locationId,mode:0})
        }
        ...
        //init
        locSer[0].queryLocationById(locationId)
            .then(function (data) {
                data = data.data;
                if(data.code === 0){
        ...
        function queryClientStep() {
            ...
            var end = $("#end").val();
            var queryInfo;
            var reg_name = /^([A-F\d]{2}:){5}[A-F\d]{2}$/;
            if (mac == "") {
            ...
            $('.box').hide();
            locSer[0].queryClientStep(queryInfo)
                .then(function (data) {
                    data = data.data;
                    if(data.code === 0 && data.clientList.length !== 0){
        ...
        function queryRealityClient(mac) {
            ...
            locSer[0].queryRealTimeClient(queryInfo)
                .then(function (data) {
                    data = data.data;
                    if(data.code === 0 && data.clientList.length !==0){
                        showClientInfo(data.clientList);
