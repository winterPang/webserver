;(function ($) {
    var MODULE_BASE = "drs";
    var MODULE_NAME = MODULE_BASE + ".dpi_ipv6";
   ...
   /* function initAweekUserFlow(nPktDir,sStartTime,sEnd)
    {
        ...
        { 
            var aUserInterface = Utils.Request.getTableRows (NC.aUserInterfaces, oInfos) || [];
            var aUser = Utils.Request.getTableRows (NC.Users, oInfos) || [];
            var nLenght = g_AweekUserApp.length;
            for(var i=0;i<aUserInterface.length;i++)
            {
                g_AweekUserApp[nLenght+i] = aUserInterface[i];
            }
            g_nCount++
            if(g_nCount==7)
            {
        ...
        var nPktDir = nPktDir;
        var nIpType = "1"; 
        var oUserSelects = Utils.Request.getTableInstance (NC.aUserInterfaces);
    ...
    {    
        ...
        //$("#dropFlow_pie").echart("init", Downoption,oTheme);
  ...
  /*  function initAweekUser(sStartTime,sEnd)
    {
        ...
        function myCallback (oInfos)
        {
            function SecondCb(oSecondInfos)
            {
                if(nPktDir == "0")
                { 
                   g_aUserDown = Utils.Request.getTableRows (NC.UserApps, oSecondInfos) || []; 
                }
                else
                {
                    g_aUserUp = Utils.Request.getTableRows (NC.UserApps, oSecondInfos) || [];  
                }
            ...
            if(nPktDir == "0")
            {
                g_aUserUp = Utils.Request.getTableRows (NC.UserApps, oInfos) || [];
                var oUserSelects = Utils.Request.getTableInstance (NC.UserApps);
                oUserSelects.addFilter({StartTime:sStartTime,EndTime:sEnd, PktDir: "1", AddressType:"1"});
                aRequest1.push(oUserSelects);
                Utils.Request.getBulk (aRequest1, SecondCb); 
            }
            if(nPktDir=="1")
            {
                g_aUserDown = Utils.Request.getTableRows (NC.UserApps, oInfos) || [];
                var oUserSelects = Utils.Request.getTableInstance (NC.UserApps);
                oUserSelects.addFilter({StartTime:sStartTime,EndTime:sEnd, PktDir: "0", AddressType:"1"});
                aRequest1.push(oUserSelects);
                Utils.Request.getBulk (Request1, SecondCb);
            }
        }
        ...
        var nPktDir = nPktDir||"0";
        var oUserSelects = Utils.Request.getTableInstance (NC.UserApps);
        oUserSelects.addFilter({StartTime:sStartTime,EndTime:sEnd, PktDir: nPktDir, AddressType:"1"});
  ...
  /*  function initAweekStatistics_URL(sStartTime,sEnd,nPktDir)
    ...
    function initWecomPie_App(aData)
    {
       ...
       // $(".page-bar", g_jForm).addClass("hide");
    } 
  ...
  /*  function initWecomApp_Chart()
    ...
    function initForm()
    {
        ...
        $("#interface_detail,#appinfo_detail,#urlinfo_detail,#userFlow_detail").on("click", function(){
            Utils.Base.redirect ({np:"drs.index_ipv6",ID:$(this).attr("id")});
            return false;
        });
        //选择点击控件
