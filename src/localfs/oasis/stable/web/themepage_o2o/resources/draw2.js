    ...
    function ($scope, $http, $window, $location, $sce) {
        ...
        $scope.publishData = function () {
            ...
            $http({
                ...
                alert('记录不存在或者网络原因，请联系管理员。erreo');
            });
        }
...
function uploadImg(imgId) {
    // 记录当前操作的图片域id
    canvasImgId = imgId;
    openwindow('/oasis/stable/web/themepage_o2o/upload.html?pathName=' + pathName, 'upload', 420, 250);
}

//轮播图增加一个空位
// function ImgAdd(){
//     var itemdiv = '<div class="item"><div id="'+canvasImgId+'" class="banner" style="position: relative;"></div></div>';
//     var lidiv = '<li data-target="#myCarousel" data-slide-to="'+canvasImgId[canvasImgId.length-1]+'"></li>';
//     jq('.carousel-inner').append(itemdiv);
//     jq('.carousel-indicators').append(lidiv);
// }
...
// function ImgDel(){
//     var imgdiv = jq(".item");
//     jq.each(imgdiv, function(Key, value) {
//         var divhtml = jq(value).html();
//         var v = divhtml.indexOf(deltitle);
//         if(v>0){            
//         }else{
//             jq(value).remove();
//         }
