...
(function (jq, undefined) {
    ...
    CanvasUi.prototype.showCfg = function () {
        ...
        var addImg = jq('<a href="#" style="margin-left:10px;"><i class="fa fa-plus-circle fa-lg"></i>&nbsp;增加</a>');
                ...
                jq('.canvas-widget-content').find("a[name=canvasFile]").unbind().bind({
                    click: function () {
                        if (!confirm('确定删除吗？')) {
                            ...
                            // deltitle = jq(this).parent().siblings('.canvas-imgcontent-left').find('img').attr('id');
                            // deltitle = deltitle.substring(0,deltitle.length-4);
