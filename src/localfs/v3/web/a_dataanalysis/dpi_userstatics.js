;(function ($) {
   ...
   /* function addUser()
    ...
    { 
        ...
        var adata = [{name:"新用户",value:12,itemStyle:labelNew},
         {name:"老用户",value:21,itemStyle:labelold}];
         /*注：下面圆环里面的比例要自己算*/
         var nval = adata[0].value;
        var labelFromatter = {
            normal: {
                label: {
                    formatter:nval + '%',
    ...
    {
       // var adata = [{name:"楼盘1",value:23},{name:"楼盘2",value:24},{name:"楼盘3",value:"25"},{name:"楼盘4",value:41},{name:"楼盘5",value:25}]; 
        var adata = aData;
        var option = {
            height: 200,
            tooltip: {
   ...
   /* function UserTrailMap()
        ...
        /*需要下发的消息*/
         var SendMsg = {
            GetUrlPersonNumChart:{
                url: MyConfig.path + "/ant/read_url_app",
                dataType: "json",
                type: "post",
                data: {
         ...
         {
            var aAppdata = data.message;
            //drowUserChangeChart(aUrldata,aAppdata);
         }
    ...
    function initHeadMsg(StartTime,EndTime)
    {
         ...
         /*获取用户数量需要下发的请求信息*/
          var SendMsg = {
            TotalNumAJAX:{
                url: MyConfig.path + "/useridentity/getOnlineUser",
                dataType: "json",
                type: "post",
                data: { 
                    devSN:g_sn,
                ...
                onSuccess:TotalNuumgetMsgSuccess,
                onFailed:TotalNumgetMsgFail
            },
            ManNumAJAX:{
                url: MyConfig.path + "/useridentity/getOnlineUser",
                dataType: "json",
                ...
                onSuccess:manNumgetMsgSuccess,
                onFailed:manNumgetMsgFail
            },
            WomanNumAJAX:{
                url: MyConfig.path + "/useridentity/getOnlineUser",
                dataType: "json",
                ...
                onSuccess:womanNumgetMsgSuccess,
                onFailed:womanNumgetMsgFail
            },
            UnknowNumAJAX:{
                url: MyConfig.path + "/useridentity/getOnlineUser",
                dataType: "json",
                ...
                onSuccess:unknowNumgetMsgSuccess,
                onFailed:unknowNumgetMsgFail
            }
           
        }
        /*发送请求获取所有用户数量，男用户数量，女用户数量，未知用户数量*/
        /*获取总用户数量*/ 
        Utils.Request.sendRequest(SendMsg.TotalNumAJAX);
        function TotalNuumgetMsgSuccess(data)
        { 
            var TotalUserNum = data.totalCount; 
            $("#TotalUserNum").html(TotalUserNum);
            
        }
        function TotalNumgetMsgFail()
        {
            console.log("fail terminal fail!");
        }  
        /*获取男用户数量*/ 
        Utils.Request.sendRequest(SendMsg.ManNumAJAX);
        function manNumgetMsgSuccess(data)
        { 
            var manUserNum = data.totalCount;  
            $("#manUserNum").html(manUserNum); 
        }
        function manNumgetMsgFail()
        {
            console.log("fail terminal fail!");
        }  
        /*获取女用户数量*/ 
        Utils.Request.sendRequest(SendMsg.WomanNumAJAX);
        function womanNumgetMsgSuccess(data)
        { 
            var womanUserNum = data.totalCount;  
            $("#womanUserNum").html(womanUserNum); 
        }
        function womanNumgetMsgFail()
        {
            console.log("fail terminal fail!");
        }  
        /*获取未知用户数*/ 
        Utils.Request.sendRequest(SendMsg.UnknowNumAJAX);
        function unknowNumgetMsgSuccess(data)
        { 
            var UnknowSexNum = data.totalCount;  
            $("#UnknowSexNum").html(UnknowSexNum);
        }
        function unknowNumgetMsgFail()
        {
            console.log("fail terminal fail!");
        }  
         
        $("#TotalUserNum").html("50");
        $("#manUserNum").html("20");
        $("#womanUserNum").html("20");
        $("#UnknowSexNum").html("10");
        /*发送请求获取上网人数和使用应用人数*/  
          /*将每个用户对应的mac放到数组里下发请求*/
          var aMacaddr = [];
          for(var i=0;i<g_UserMsg.length;i++)
          {
            aMacaddr.push(g_UserMsg[i].macAddr);
          }
         var SendMsg = {
            GetNetPlayNum:{
                url: MyConfig.path + "/ant/read_url_app",
                ...
                type: "post",
                data: {
                    Method: 'GetNetPlayNum', 
                    Param: {
                        family: "0",
                        direct: "0",
                        ACSNList: g_sn,
                        MACList:aMacaddr,
                        StartTime: sStartTime.getTime()/1000,
                        EndTime: sEndTime.getTime()/1000,
                    }
                },
                onSuccess:getUrlMsgSuccess,
                onFailed:getUrlMsgFail
        ...
        /*获取上网人数*/
        Utils.Request.sendRequest(SendMsg.GetNetPlayNum);
        function getUrlMsgSuccess(data)
        {
            var urlNumber = data.message;
            $("#NetPlayNum").html(urlNumber);
        }
        function getUrlMsgFail()
        {
            console.log("fail terminal fail!");
        ...
        /*获取使用应用人数*/
        Utils.Request.sendRequest(SendMsg.GetAccessAppNum);
        function getAppMsgSuccess(data)
        {
            var appNumber = data.message;
           // $("#UseAppNum").html(appNumber);
        }
        function getAppMsgFail()
        {
            console.log("fail terminal fail!");
    ...
    function UserAccessNum(StartTime,EndTime)
    {
        /*发送Ajax请求获取来访次数*/
         ...
         var sEndTime = EndTime || nowEndTime;
         var SendMsg = {
            url: MyConfig.path + "/useridentity/getUserHistroy",
            dataType: "json",
            type: "post",
            data:{ 
                /*devSN:g_sn,
        ...
        function AccessNumgetMsgSuccess(data)
        {
            ...
            var aAccessNumToBar = [0,0,0,0,0,0,0];
            for(var i=0;i<aUserList.length;i++)
            {
                if(aUserList[i].count<10)
                {
                    aAccessNumToBar[0]++;
                }
                if(aUserList[i].count>=10 && aUserList[i].count < 20)
                {
                    aAccessNumToBar[1]++;
                }
                if(aUserList[i].count>=20 && aUserList[i].count < 30)
                {
                    aAccessNumToBar[2]++;
                }
                if(aUserList[i].count>=30 && aUserList[i].count < 40)
                {
                    aAccessNumToBar[3]++;
                }
                if(aUserList[i].count>=40 && aUserList[i].count < 50)
                {
                    aAccessNumToBar[4]++;
                }
                if(aUserList[i].count>=50 && aUserList[i].count < 100)
                {
                    aAccessNumToBar[5]++;
                }
                if(aUserList[i].count>=100)
                {
                    aAccessNumToBar[6]++;
                }
            }
    ...
    function UserVisiteCount()
    {
        var adata = [{name:"1次",value:100},{name:"2次",value:21},{name:"3次",value:32},{name:"4次",value:89},{name:">=5次",value:33}];
    ...
    function AjaxGetUserMsg(StartTime,EndTime)
    {
         var nyear = new Date().getFullYear();
         var nmonth = new Date().getMonth();
         var nday = new Date().getDate()+1;
            ...
            for(var i=0;i<length;i++)
            {
               ...
               // g_UserMsg.push(otemp);
                for(var j=0;j<g_UserMsg.length;j++)
                {
                    ...
                    {
                        //g_UserMsg.name = otemp.name;
                        var macLength = g_UserMsg[i].macAddr.length;
                         /*删除每个用户中MAC地址重复的*/
                        for(var k=0;k<macLength;k++)
                        {
                            if(g_UserMsg[i].macAddr[k] == otemp.macAddr[0])
                            {
                                break;
                            }
                            if(k == macLength-1)
                            {
                                g_UserMsg.macAddr.push(otemp.macAddr[0]);
                            }
                        }
    ...
    function ClickFilter()
    {
        ...
        switch(value)
        {
            case 0:
            {
                ...
                var endTime = getTheDate("one","start");
                var sStarttime = new Date(startTime);
                var sEnd = new Date(endTime);
                AjaxGetUserMsg(sStarttime,sEnd);
                initHeadMsg(sStarttime,sEnd);
                UserAccessNum(sStarttime,sEnd);
            ...
            case 1:
            {
                ...
                var endTime = getTheDate("aweek","start");
                var sStarttime = new Date(startTime);
                var sEnd = new Date(endTime);
                AjaxGetUserMsg(sStarttime,sEnd);
                initHeadMsg(sStarttime,sEnd);
            ...
            case 2:
            {
                ...
                var endTime = getTheDate("month","start");
                var sStarttime = new Date(startTime);
                var sEnd = new Date(endTime);
                AjaxGetUserMsg(sStarttime,sEnd);
                initHeadMsg(sStarttime,sEnd);
            ...
            case 3:
            {
                ...
                var endTime = getTheDate("year","start");
                var sStarttime = new Date(startTime);
                var sEnd = new Date(endTime);
                AjaxGetUserMsg(sStarttime,sEnd);
                initHeadMsg(sStarttime,sEnd);
            ...
            default:
            {
                ...
                var sStarttime = new Date(sNowStart);
                var sEnd = new Date(sNowEndTime);
                AjaxGetUserMsg(sStarttime,sEnd);
                initHeadMsg(sStarttime,sEnd);
                UserAccessNum(sStarttime,sEnd);
                dpiData(startTime,endTime);
