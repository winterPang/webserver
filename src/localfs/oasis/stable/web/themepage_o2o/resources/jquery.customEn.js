...
(function (jq, undefined) {
    ...
    CanvasControl.prototype.getFinalHtml = function () {
        ...
        dom.find("#phoneTgudingForm").hide();
        // 去掉文字
        var pageType = jq.getUrlParams('type');
        if (pageType == 1) {
            i18_init_index_simplel(dom);
        } else if (pageType == 2) {
            i18_init_login_simplel(dom);
        } else if (pageType == 3) {
            i18_init_loginSuc_simplel(dom);
        } else {
            i18_init_home_simplel(dom);
        }
        return dom.html();
    }
    ...
    CanvasUi.prototype.addMenu = function () {
        var parent = this;
        //增加文字消息
        this.canvasMenuText = jq('<form id="canvasMenuText" name="canvasMenuText" method="post" title="Text" class="mousepointer"><img src="../resources/images/text.png"  style="vertical-align:middle;"><span>Text</span></form>');
        this.canvasMenuText.unbind("click").click(function (event) {
            var wenziMenu = jq('<div id="text' + jq.getRandomDateBetween(new Date()) + '" class="canvas-edited-text canvas-event-selectable canvas-event-draggable '
                ...
                + 'relative; word-break: break-word"><span word-break:break-word;><p style="text-align: left;">Text area</p></span></div>');
        ...
        //增加图文消息
        this.canvasMenuImg = jq('<form id="canvasMenuImg" name="canvasMenuImg" method="post" style="margin-top:20px;width:100px;" title="Picture"  class="mousepointer" ><img src="../resources/images/img.png" style="vertical-align:middle;" ><span>Picture</span></div></form>');
        this.canvasMenuImg.unbind("click").click(function (event) {
            ...
            if (undefined != canvasControl.canvasContent.find("div[class*='canvas-edited-text']").last() &&
                ...
                canvasControl.canvasContent.find("div[class*='tpAdDiv']").first().append(imgMenu);
            }
        });
        ...
        if (jqElm.hasClass("canvas-event-click")) {
            jqElm.bind({
                showSelected: function () {
                    ...
                    var toolbar = jq('<div class="canvas-toolbar-div" style="right:54px;"><span class="canvas-toolbar-span">Edit</span></div>');
                    toolbar.bind({
                    ...
                    var deleToolbar = jq('<div class="canvas-toolbar-div-delete"><span class="canvas-toolbar-span">Delete</span></div>');
                    deleToolbar.bind({
                        click: function (event) {
                            canvasControl.canvasUi.hideEdit();
    ...
    jq.extend(CanvasWidget.TextPanel.prototype, {
        init: function () {
            ...
            var title = jq('<div class="canvas-widget-header"><i class="widget-icon fa fa-tasks fa-lg themeprimary"></i> <span class="widget-caption themeprimary">Edit Text</span></div>');
    ...
    jq.extend(CanvasWidget.ImgPanel.prototype, {
        init: function () {
            ...
            var addBtn = jq('<a href="#" style="margin-left:10px;"><i class="fa fa-plus-circle fa-lg"></i>&nbsp;Add</a>').unbind().bind({
                click: function () {
                    ...
                    parent.iCount++;
                    var tool1 = jq('<div class="canvas-eachItem">' +
                        '<div class="canvas-imgcontent-left"><img title="Upload picture" id="' + canvasControl.canvasUi.curElm.attr("id") + '_' + jq.getRandomDateBetween(new Date()) + '" class="getImg" src="../resources/images/add.gif" width="100px" height="100px"/></div>' +
                        '<div class="canvas-imgcontent-right">' +
                        '<div style="height:45px;margin-left:5px;">' +
                        'Show time：<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt"/>-<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt"/>' +
                        '</div>' +
                        '<div style="height:50px;margin-left:5px;">' +
                        'Picture link：<input type="text" maxlength="128" placeholder="Start with http or https" class="canvas-text-80 getTxt"/>' +
                        '</div>' +
                        '</div>' +
                        '<div class="canvas-imgdel-link"><a href="#" name="canvasFile"><i class="fa fa-trash-o fa-lg"></i></a></div>' +
                        '</div>');
                    var tool2 = jq('<div class="canvas-eachItem">' +
                        '<div class="canvas-imgcontent-left"><img title="Upload picture" id="' + canvasControl.canvasUi.curElm.attr("id") + '_' + jq.getRandomDateBetween(new Date()) + '" class="getImg" src="../resources/images/add.gif" width="100px" height="100px"/></div>' +
                        '<div class="canvas-imgcontent-right">' +
                        '<div style="height:45px;margin-left:5px;">' +
                        'Show time：<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt"/>-<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt"/>' +
                        '</div>' +
                        '<div style="height:50px;margin-left:5px;display:none;">' +
                        'Picture link：<input type="text" maxlength="128" placeholder="Start with http or https" class="canvas-text-80 getTxt"/>' +
                        '</div>' +
                        '</div>' +
                        '<div class="canvas-imgdel-link"><a href="#" name="canvasFile"><i class="fa fa-trash-o fa-lg"></i></a></div>' +
                        '</div>');
                    if (Type == 1 || Type == 2) {
                        canvasControl.canvasUi.imgPanel.iContent.append(tool2);
                    } else {
                        canvasControl.canvasUi.imgPanel.iContent.append(tool1);
                    }
                    parent.addEvent();
                }
            });
                ...
                click: function () {
                    if (!confirm('Are you sure want to delete the item?')) {
                        ...
                        if (1 == canvasControl.canvasUi.imgPanel.iCount) {
                            alert('Can\'t delete the picture because at least one picture is required.');
                            return;
                        }
        ...
        setContent: function () {
            ...
            // 从url后面获取templateId,表示第几个页面
            Type = jq.getUrlParams('type');
            if ('' != content && null != content && undefined != content) {
                // 遍历数据，然后分别显示每个上传文件内容
                for (var i in content) {
                    parent.iCount++;
                    content[i][0] = content[i][0].replace(tracker + '/', '');
                    var tool3 = jq('<div class="canvas-eachItem">' +
                        '<div class="canvas-imgcontent-left"><img title="Upload picture" id="' + canvasControl.canvasUi.curElm.attr("id") + '_' + jq.getRandomDateBetween(new Date()) + '" class="getImg" src="' + (undefined == content[i][0] ? "" : tracker + '/' + content[i][0]) + '" width="100px" height="100px"/></div>' +
                        '<div class="canvas-imgcontent-right">' +
                        '<div style="height:45px;margin-left:5px;">' +
                        'Show time：<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt" value="' + (undefined == content[i][1] || "-1" == content[i][1] ? "" : content[i][1]) + '"/>-<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt" value="' + (undefined == content[i][2] || "-1" == content[i][2] ? "" : content[i][2]) + '"/>' +
                        '</div>' +
                        '<div style="height:50px;margin-left:5px;">' +
                        'Picture link：<input type="text" maxlength="128" placeholder="Start with http or https" class="canvas-text-80 getTxt" value="' + (undefined == content[i][3] || "-1" == content[i][3] ? "" : content[i][3]) + '"/>' +
                        '</div>' +
                        '</div>' +
                        '<div class="canvas-imgdel-link"><a href="#" name="canvasFile"><i class="fa fa-trash-o fa-lg"></i></a></div>' +
                        '</div>');
                    var tool4 = jq('<div class="canvas-eachItem">' +
                        '<div class="canvas-imgcontent-left"><img title="Upload picture" id="' + canvasControl.canvasUi.curElm.attr("id") + '_' + jq.getRandomDateBetween(new Date()) + '" class="getImg" src="' + (undefined == content[i][0] ? "" : tracker + '/' + content[i][0]) + '" width="100px" height="100px"/></div>' +
                        '<div class="canvas-imgcontent-right">' +
                        '<div style="height:45px;margin-left:5px;">' +
                        'Show time：<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt" value="' + (undefined == content[i][1] || "-1" == content[i][1] ? "" : content[i][1]) + '"/>-<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt" value="' + (undefined == content[i][2] || "-1" == content[i][2] ? "" : content[i][2]) + '"/>' +
                        ...
                        '</div>');
                    if (Type == 1 || Type == 2) {
                        canvasControl.canvasUi.imgPanel.iContent.append(tool4);
                    } else {
                        canvasControl.canvasUi.imgPanel.iContent.append(tool3);
                    }
                    parent.addEvent();
                }
            } else {
                // 仅显示一个文件上传
                parent.iCount++;
                var tool5 = jq('<div class="canvas-eachItem">' +
                    '<div class="canvas-imgcontent-left"><img title="Upload picture" id="' + canvasControl.canvasUi.curElm.attr("id") + '_' + jq.getRandomDateBetween(new Date()) + '" class="getImg" src="../resources/images/add.gif" width="100px" height="100px"/></div>' +
                    '<div class="canvas-imgcontent-right">' +
                    '<div style="height:45px;margin-left:5px;">' +
                    'Show time：<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt"/>-<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt"/>' +
                    ...
                    '<div class="canvas-imgdel-link"><a href="#" name="canvasFile"><i class="fa fa-trash-o fa-lg"></i></a></div>' +
                    '</div>');
                var tool6 = jq('<div class="canvas-eachItem">' +
                    '<div class="canvas-imgcontent-left"><img title="Upload picture" id="' + canvasControl.canvasUi.curElm.attr("id") + '_' + jq.getRandomDateBetween(new Date()) + '" class="getImg" src="../resources/images/add.gif" width="100px" height="100px"/></div>' +
                    '<div class="canvas-imgcontent-right">' +
                    '<div style="height:45px;margin-left:5px;">' +
                    'Show time：<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt"/>-<input type="text" readonly class="ui_timepicker canvas-text-35 getTxt"/>' +
                    ...
                    '</div>');
                if (Type == 1 || Type == 2) {
                    canvasControl.canvasUi.imgPanel.iContent.append(tool6);
                } else {
                    canvasControl.canvasUi.imgPanel.iContent.append(tool5);
                }
                parent.addEvent();
            }
