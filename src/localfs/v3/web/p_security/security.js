...
(function ($)
{
    ...
    function initData(jScope)
    {
        function myCallback(oInfos)
        {
            ...
            $("#WipList").SList ("refresh", g_AllRadio);
        }
    ...
    function initGrid()
    {
        ...
        var opt = {
            colNames: getRcText ("LIST_TITLE_WIP"),
            showHeader: true,
            multiSelect: true,
            pageSize:10,
            colModel: [
        ...
        $("#WipList ").on('click','a.list-link', function(){
            var sAP = $(this).attr('ap');
            var sRadio = $(this).attr('radio');
            var aStr = getRcText("ON_OFF").split(",");
            var oInfor = {
        ...
        $('#alldevice').on("click",function(){
            $('#WipsList').show().SList("resize");
            $('#WipList').hide();
        });
        $('#singledevice').on("click",function(){
            $('#WipList').show().SList("resize");
            $('#WipsList').hide();
        });
    }
