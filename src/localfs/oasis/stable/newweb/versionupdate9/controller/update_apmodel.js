 ...
 */
define(['echarts', 'utils', 'common9/devices', 'css!versionupdate9/css/update_model'], function (ecahrts, Utils, Dev) {
...
*/
            ...
            var URL_GET_DEV = "/v3/ace/oasis/oasis-rest-shop/restshop/shopModel/getdevicesbyshopidanddevmodel?shop_id="+shopId+"&dev_model="+model;
            ...
            $scope.upgrad = {
                ...
                showPageList: true,
                columns: [
                    {field: 'devAlias', title: "设备名称"},
                    {field: 'devSn', title: "设备序列号"},
                    {
                        field: 'softVer', title: "当前版本",
                        formatter: function (val, row) {
                            return val ? val : row.softwareVersion ? row.softwareVersion.split(',')[1].trim() : '';
                        }
                    },
                    {field: 'latestVersion', title: "最新版本"},
                    {
                        field: 'lastUpdateTime', title: "上次升级时间",
                        formatter: function (val) {
                            return val ? $filter('date')(val, 'yyyy-MM-dd HH:mm:ss') : '';
                        }
                    },
                    {
                        searcher: {}, field: 'percent', title: "下载进度", color: "green",
                        formatter: function (value, row, index) {
                            var id = row.apSn;
                            if (value === 0) {
                                return '<div>未下载</div>';
                    ...
                    {
                        searcher: {}, field: 'status', title: "升级状态",
                        formatter: function (value, row, index) {
                            return updateErr[value];
                        }
                    }
            ...
            function loadUpgradTable(){
                $http.get(URL_GET_DEV).success(function (data) {
                    if(data.code == 0){

                    }
                    $scope.$broadcast('load#upgradTable', data.data);
                });
            }
            $timeout(function () {
                loadUpgradTable();
