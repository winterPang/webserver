(function ($)
{
    ...
    {
        if(undefined != adata.errCode)
        {
            g_resultSatus[modules] = adata.errCode;
        }
        else
        {
    ...
    function showMsg()
    {
        if((g_resultSatus.wips == 0) && (g_resultSatus.natdetect == 0) &&(g_resultSatus.probe == 0))
        {
            Frame.Msg.info("配置成功！");
            return;
        }
        ...
        {
            Frame.Msg.error("无线安全：" + g_resultError[g_resultSatus.wips] + ';\n' + 
                    "无线探针：" + g_resultError[g_resultSatus.probe] + ';\n' + 
                    "私接代理检测：" + g_resultError[g_resultSatus.natdetect] + '.\n'  
                );
            return;
        }
    ...
    function initGrid()
    {
        ...
        if (-1 != aPermission.indexOf("CONFIG_WRITE")) {
            var aButtons = [
