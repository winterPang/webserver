...
(function(jq, undefined) {
    ...
    */
    ...
    CanvasControl.prototype.getSelect = function() {
        return canvasControl.select;
    }
    ...
    CanvasControl.prototype.getFinalCfg = function() {
        ...
        canvasControl.canvasForm.find("div.canvas-eachItem").each(function(i){
			    ...
			    //判断是否是带有标题，内容的图片
			    var isisLogo2 = jq(this).find('.getImg').attr("imgcfg").indexOf("isLogo=2");
			    if(isisLogo2 > 0){
			    ...
			    var isLogo4 = jq(this).find('.getImg').attr("imgcfg").indexOf("isLogo=4");
			    if(isLogo4 > 0){
			    	var title = jq(this).find('.getTitle').val();
			    	aArray.push(title);
			        jq(canvasControl.canvasContent).find("#"+key+"Title").html(title);
        ...
        var teltype = jq.getUrlParams("type");
	    if(teltype == 4 || teltype == 41 || teltype == 42 || teltype == 43){
	    	var newTel = jq("#getTel").val();
	        jq("#homeTel").text(newTel);
	        jq("#homeTel").attr("href","tel:"+newTel)
	        var aArray = new Array();
	        aArray.push("-1");
	        aArray.push("-1");
	        aArray.push("isTel");
	        aArray.push(newTel);
	        newJsonCfg["homeTel"] =aArray;
	        
		    switch(teltype){
		    	case "41":newJsonCfg["selected"] = homeOne;  break;
		    	case "42":newJsonCfg["selected"] = homeTwo;  break;
		    	case "43":newJsonCfg["selected"] = homeThree;break;
		    	default: newJsonCfg["selected"] = homeOne;
		    }
		    
        }
	    if(teltype == 2 || teltype == 21 || teltype == 22){
	    	 switch(teltype){
		    	case "21":newJsonCfg["selected"] = loginOne;  break;
		    	case "22":newJsonCfg["selected"] = loginTwo;  break;
		    	default: newJsonCfg["selected"] = loginOne;
		    }
    ...
    //{"image100":["images/bgImg.jpg","http://www.baidu.com"],"logoIndex100":["images/logo_h3c.png","http://www.baidu.com"]}
    CanvasUi.prototype.showCfg = function(){
    	...
    	for(var id in cfg){
    		if(id == "selected"){
    			canvasControl.select = cfg[id];
    			jq(("input[value='"+cfg[id]+"']")).attr("checked",'checked');
    			if(cfg[id] == "loginTwo"){
    				this.showHtmlImg();
    			}
    			continue;
	    	...
	    	var item2 = jq('<div class="canvas-eachItem">' +
					  ...
					  '</div><div class="canvas-imgcontent-right">' +
					    ...
					    '<span style="height:30px;margin-left:5px;cursor:pointer" onclick=clearImg('+JSON.stringify(id)+') >' +
					  ...
					  '</div>' +
					'</div>');
	    	 var item4 = jq('<div class="canvas-eachItem">' +
	  				  '<div class="canvas-imgcontent-left">'+
	  				'  <img title="上传图片" id="'+id+'edit'+'" onclick=uploadImg('+JSON.stringify(id)+',"'+cfg[id][2]+'") class="getImg" src="'+cfg[id][0]+'" width="100px" height="100px" style="cursor:pointer" imgcfg="'+cfg[id][2]+'"/>'+
					  '</div><div class="canvas-imgcontent-right">' +
					  
					  '<div style="height:30px;margin-left:5px;">' +
                     '标题：<input type="text" maxlength="64" class="canvas-text-50 getTitle" value="'+cfg[id][3]+'"/>' +
                     '</div>' +
	    	...
	    	if(cfg[id][2].indexOf("isLogo=3") > 0){
	    		this.imgPanel.append(item1);
	    	}
	    	if(cfg[id][2].indexOf("isLogo=4") > 0){
	    		this.imgPanel.append(item4);
	    	}
	    	
	    	if(cfg[id][3] != undefined && cirIndex > 0 || cfg[id][3] != undefined && cfg[id][3] == "text100"){
	    	...
	    	/*style="height:50px;margin-left:5px;padding-top:20px;"*/
	    	if(cfg[id][2] == "isTel"){
	    		var itemTel = jq('<div class="canvas-eachItem">' +
							        'tel：<input type="text" maxlength="128" class="canvas-text-80" id="getTel" value="'+cfg[id][3]+'"/>' +
							'</div>');
	    		jq('#texttools').append(itemTel);
	    		jsonCfgText[id] = cfg[id][3];
	    	}
    	}
