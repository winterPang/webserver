;(function($)
{
    ...
    function scrollTo(jContainer, jEle)
    {
        var nOffsetTop =jEle.offset().top;
        if(jContainer.is("#page_container"))
        {
            nOffsetTop -= 200;
        }
        jContainer.scrollTop(nOffsetTop);
    }
    ...
    function getCurNode()
    {
        var oNode = JSON.parse(Frame.Cookie.get("TreeNode"));

        oNode = oNode || {};

        oNode.nodeId =  oNode.nodeId || "_TreeRoot";
        oNode.nodeType =  oNode.nodeType || "root";
        oNode.menuType =  oNode.menuType || "M_Global";
        oNode.RadioNum =  oNode.RadioNum || "0";
    ...
    function reloadTree(sMenu)
    {
        setTimeout(function(){
            Frame.NewMenu.reloadTree(sMenu);
        },500);
    }
...
function showError(msg, jScope)
{
    ...
    else if(("" != sErrMsg) && (Utils.Msg) && (Utils.Msg.error) )
    {
...
var oBase = {
    ...
    setMenuPath: function(sPath){return Frame.NewMenu.setMenuPath(sPath)},
    addMenuPath: function(sPath){return Frame.NewMenu.addMenuPath(sPath)},
    popMenuPath: function(){return Frame.NewMenu.popMenuPath()},
