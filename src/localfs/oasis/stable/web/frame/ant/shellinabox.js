...
var ATTR_DEFAULT   =0x60F0
var ATTR_REVERSE   =0x0100
var ATTR_UNDERLINE =0x0200
var ATTR_DIM       =0x0400
var ATTR_BRIGHT    =0x0800
var ATTR_BLINK     =0x1000
var ATTR_DEF_FG    =0x2000
var ATTR_DEF_BG    =0x4000
...
VT100.prototype.showContextMenu = function(x, y) {
  this.menu.innerHTML         =
    '<table class="popup" ' +
      ...
      '<tr><td>' +
        '<ul id="menuentries">' +
          '<li id="beginclipboard">复制</li>' +
          '<li id="endclipboard">粘贴</li>' +
          '<li id="browserclipboard">从浏览器粘贴</li>' +
          '<hr />' +
          '<li id="reset">重置</li>' +
          '<hr />' +
          '<li id="beginconfig">' +
...
function ShellInABox(url, container) {
  if (url == undefined) {
    ...
    this.url        = document.location.origin+ "/v3/ant/confmgr";
    this.ACSN       = document.location.search.replace(/[^0-9a-zA-Z]+/ig,"");
    // this.ACSN       = document.location.search.match(new RegExp("[\?\&]sn=([^\&]+)","i"))[1];
  } else {
    this.rooturl    = url;
    this.url        = url;
  }
  ...
  setTimeout(function(shellInABox) {
               return function() {
                 ...
                 shellInABox.showReconnect(true);
               };
             }(this), 1);
};
...
//	cmdProxy:""
//};
//this.session ? (content.session = this.session) : 0;
  ...
  if (request.readyState == XHR_LOADED) {
    if (request.status == 200) {
      ...
      if (response.data) {
        ...
        // {
        //   response.data = response.data.replace(/\r/g, "\r\n");
        // }
      ...
      if (!response.session ||
        ...
        if(this.session)
        {
          // window.parent.document.getElementById("session_father_saved").setAttribute("value", response.session);
        }
...
setInterval(function(){
	var newVlue = document.getElementById("commandline_input").getAttribute("value");
	if( newVlue != g_value)
	{
		window.shellinabox.keysPressed(newVlue + (window.shellinabox.crLfMode?"\r\n":"\r"));
		document.getElementById("commandline_input").setAttribute("value", "");
	}
},100);
