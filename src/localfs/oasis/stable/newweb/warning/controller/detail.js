define(['utils', 'moment', 'echarts3', 'css!warning/css/style','filesystem/libs/oasis-select'], function (Utils, moment, echarts) {
    return ['$rootScope', '$scope', '$http', '$alertService', function ($rootScope, $scope, $http, $alert) {
        ...
        $scope.list = {};
        $scope.modal = {};
        $scope.$watch('oasisType',function(){
            $scope.showRegionFilter=($rootScope.oasisType==1?true:false);
        });
        $scope.filterBar = {
        ...
        $scope.config = {
            ...
            module: [
                {name: 'AP', state: false},
                {name: 'STA', state: false},
                {name: 'DHCP', state: false},
                {name: 'APMGR', state: false},
                {name: 'SHELL', state: false}
            ]
        };
        ...
        function initPopover() { // 提示框
            $('#configBtn').popover({
                ...
                template: '<div class="popover my-popover" role="tooltip">' +
                '<div class="arrow"></div>' +
        ...
        function initEvent() {
            ...
            $scope.config.clickCollectedSwitch = function () { // 开启/关闭 日志收集
                if ($scope.config.collected) {
                    ...
                    $scope.config.level = '';
                    angular.forEach($scope.config.module, function (item) {
                        item['state'] = false;
                    });
                }
            };

            $scope.config.clickModuleBtn = function (method, module) {
            ...
            $scope.config.selectLevelDes = function (level) {
                if ($scope.config.collected) {
