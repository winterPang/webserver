;(function($)
{
	...
	function checkEleEvenNum(jEle, aParas)
	{
		var sValue = $.trim(jEle.val());
		var iMin = parseInt(jEle.attr("min"));
		var iMax = parseInt(jEle.attr("max"));
	...
	{
		var sValue = $.trim(getJEleValue(jEle));
		var sErrorIp = "192.0.0.0";

		if ((!isIp4Addr(sValue)) || (sErrorIp == sValue))
		{
			return "ipv4";
		}
	...
	function autoCheckForm(jForm)
	{
		...
		$("textarea, input, select, [ctrl]", jForm).bind("focusout", function (evetn)
		{
			checkEle($(this));
			return;
		});
		$(".singleSelect", jForm).bind("change", function (evetn)
		{
			checkEle($(this));
			return;
		});
