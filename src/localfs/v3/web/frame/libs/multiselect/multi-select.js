...
!function ($) {
  ...
  * ====================== */
  ...
  var MultiSelect = function (element, options) {
    ...
    this.$container = $('<div/>', { 'id': "ms-"+id, 'class': "ms-container row-fluid" });
  ...
  MultiSelect.prototype = {
    ...
    init: function(){
      ...
      if (ms.next('.ms-container').length == 0){
        ...
        that.$selectionUl.on(action, '.ms-input', function(e){
          e.stopPropagation();
        });
    ...
    'select' : function(svalue, method){
      var value,aItems=[];
      var bItemisNotNull = false;
      var sCustomField = this.options.customField;
      if(svalue.length == 0){return;}
      if (typeof svalue[0] === 'object')
      {
          value = [];
          $.each(svalue, function(i,oData){
              value.push(oData.value);
              aItems[oData.value] = oData[sCustomField];
              bItemisNotNull = true;
          });
      }
      else
      ...
      {
        var aValue = this.$element.val()||[];
        var nMax = this.options.max;
        if((value.length>nMax)||(aValue.length>=nMax))
        {
          return false;
        }
      }
      ...
      if (selectables.length > 0){
        ...
        if("" != that.options.customField)
        {
            selections.each(function(){
                if(""!=that.options.selectInner)
                {
                    $(that.options.selectInner).addClass("ms-custom").appendTo(this);
                };
                var item = $(this).attr("customitem");
                if(item)
                {
                    $(this).find(".ms-input").val(item);
                }
            });              
            if(bItemisNotNull)
            {
                selections.each(function(i){
                    $(this).find(".ms-input").val(aItems[msIds[i]]);
                    $(this).attr("customitem",aItems[msIds[i]]);
                    $(selectables[i]).attr("customitem",aItems[msIds[i]]);
                });  
            }
        }
