(function($) {
    ...
    function showSSID(oRowdata, sName) {//拿到点击的那一行数据
        ...
        function onSubmitSSID() {
            function operatePubMng(loginPage, authCfg) { //3.选登录模板 更改发布模板 
                ...
                if (!oRowdata.pubMngName || !("pubMngName" in oRowdata)) { //发布模板不存在 创建发布模板
                    getDeviceInfo().done(function(data, textStatus, jqXHR) {
                        ...
                        //添加发布模版
                        addPubMng(name, shopName, ssidName, WeChart, authCfg, loginPage, g_oPara.ssidId).done(function(data, textStatus, jqXHR) {
                            ...
                            //发布 发布模版
                            PublishPubMng(name, true).done(function(data, textStatus, jqXHR) {
                                //在这里进行判断，如果以前有发布模板并且已经发布了的话，那么就要先用SSID把原来的发布
                                //模板删除，然后再添加发布模板
                                if (data.errorcode == 1308) {
                                    PublishPubMng(name, false).done(function(data, textStatus, jqXHR) {
            ...
            if (authType == "AT1") { //1.不认证 删除发布模板 取消发布
                ...
                if (!authCfg) {
                    Frame.Msg.info(getRcText("CHECK_AUTH"),"error");
                    return;
                }
                if (loginType == "LP4") {
                    operatePubMng(loginPage, authCfg); //选登录模板 更改发布模板
        ...
        //添加发布模版
        function addPubMng(pubMngName, shopName, ssidName, WeChart, authCfg, loginPage,ssidId) {
            return $.ajax({
            type: "POST",
    ...
    function updataSSID(){
        ...
        if ($($("#cipherSuite_true")[0]).next().hasClass("checked") && $("#restByte").val()) {
            return $.ajax({
                type:"post",
    ...
    function initData() {
        $("#apName").html(g_oPara.stName);
        $("#apModel").html(g_oPara.ssidName);
                    ...
                    getSSIDInfo().done(function(data, textStatus, jqXHR) {
                        ...
                        queryPubMng().done(function(data, textStatus, jqXHR) {
                            ...
                            pubmng_list.forEach(function(pubmng) {
                                if(g_oPara.ssidId ==pubmng.ssidId && pubmng.isPublished == 1){
                                    obj[g_oPara.stName].authCfgTemplateName=pubmng.authCfgName;
                                    obj[g_oPara.stName].themeTemplateName=pubmng.themeTemplateName;
                                    obj[g_oPara.stName].pubMngName=pubmng.name;
                                    obj[g_oPara.stName].weixinAccountName=pubmng.weixinAccountName;
                                    ...
                                    $("#WeChartList").singleSelect("value",obj[g_oPara.stName].weixinAccountName);
                                }
                                auth_list.forEach(function(authmsg) {
                                    if(obj[g_oPara.stName].authCfgTemplateName==authmsg.authCfgTemplateName){
                                        

                                            for(var i=0;i<authmsg.uamAuthParamList;i++){
                                                if(authmsg.uamAuthParamList[i].authParamName=="NO_SENSATION_TIME"){
                                                    if(authmsg.uamAuthParamList[i].authParamValue!="0"){
                                                        $("#auto_study_enable").MCheckbox("setState", true);
                                                        $(".Learn-MAC", jFormSSID).show();
                                                    }
                                                    // else{
                                    ...
                                    if (obj[g_oPara.stName].authCfgTemplateName && authmsg.authCfgTemplateName == obj[g_oPara.stName].authCfgTemplateName) {
                            ...
                            if(obj[g_oPara.stName].AuthType=="不认证"||obj[g_oPara.stName].AuthType==""){
                                $("#authTypes div:first div:first div span").addClass("checked").siblings().removeClass("checked");
                            }
                            if(obj[g_oPara.stName].AuthType=="一键上网"||obj[g_oPara.stName].AuthType=="账号认证")
                            {
                                ...
                                obj[g_oPara.stName].AuthenType = 'AT4';
                            }

                            onShowAdvieace(obj[g_oPara.stName].AuthenType);
                            hPending.close();
                        })
        ...
        $("#ensure").click(function(){
            ...
            // if($("#cipherSuite input:last").next().hasClass("checked")==true){
            //     if($("#restByte").val()==""){
    ...
    function initForm() {
        var addForm = $("#toggle_form");//第一个form
        addForm.form("init", "edit", {"title":getRcText("ADD_TITLE"), "btn_apply":false, "btn_cancel":false});
        ...
        $("input[type=radio][name=AuthenType]").on("click",function(){
            var sCheck = $(this).val();
            onShowAdvieace(sCheck);
        });
