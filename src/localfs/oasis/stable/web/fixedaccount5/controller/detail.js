define(['utils', 'jquery', 'angular-ui-router', 'bsTable'], function (Utils, $) {
    return ['$scope', '$http', '$rootScope', '$alertService', '$state', function ($scope, $http, $rootScope, $alert, $state) {
        ...
        var reg = /^\+?[1-9][0-9]*$/;
        var groupId = $state.current.name == 'scene.content.monitor.fixedaccount15' ? $scope.branchName : '';

        //$scope.permission: {raw: Array[3], read: true, write: true, execute: true}
        ...
        $scope.userOption = {
            tId: 'userlist',
            url: '/v3/ace/oasis/auth-data/o2oportal/registuser/queryaccount?groupId=' + groupId + '&ownerName=' + $scope.userInfo.user + '&storeId=' + $scope.sceneInfo.nasid + '&isUsingNickName=true',
            pageSize: 10,
            showPageList: false,
            pageParamsType: 'path',
            method: 'get',
        ...
        $scope.import = {
            options: {
                ...
                okHandler: function (modal, $ele) {
                    ...
                    $http({
                        method: 'POST',
                        url: '/v3/ace/oasis/auth-data/o2oportal/registuser/upload?groupId=' + groupId + '&storeId=' + $scope.sceneInfo.nasid,
                        data: fd,
