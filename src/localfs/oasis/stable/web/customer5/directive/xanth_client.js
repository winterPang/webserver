define(['angularAMD', 'utils', 'sprintf'],
    function (app, utils) {
        ...
        var URL_TEMPLATE_FILE = sprintf('../customer5/views/%s/xanth_client.html', sLang);
                ...
                return {
                    ...
                    link: function ($scope, $element, $attrs, $ngModel) {                       
                        $scope.chouseUrl = function(mode){
				            ...
				            $http.get(customerUrl+'?devSN='+$scope.sn+'&nasId='+$scope.nasid+'&dataType='+para
				                ...
				                for (var i = 0; i < $scope.newlength; i++) {
				                	if(data.histclientList[i].totalCount!=='-' || data.histclientList[i].newCount!=='-'){
				                		g_flag=false;
				                	}
				                ...
				                $scope.myechart($scope.g_AllUser,$scope.g_newUser,$scope.g_Time,g_flag);
				            }).error(function(){

				            })
				        }
				        $scope.myechart = function(allcouver,newcouver,time,flag){
