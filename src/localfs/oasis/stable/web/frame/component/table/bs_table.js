define(['angularAMD', './frame/component/table/lang/bs_table', 'utils', 'bootstrapTable', 'select2', 'bootstrap-daterangepicker', 'bootstrapDatepicker', 'rangepicker', 'css!bootstrap_daterangepicker_css', 'css!bootstrap_table_css', 'css!./frame/component/table/less/index'], function (_angularAMD, _bs_table, _utils) {
    'use strict';

    var _angularAMD2 = _interopRequireDefault(_angularAMD);

    var _bs_table2 = _interopRequireDefault(_bs_table);
    ...
    // 表格默认值
    _angularAMD2.default.constant('bsTableDef', $.extend(true, {}, $.fn.bootstrapTable.defaults, {
        ...
        pageChangeFlag:"first", // 标识点击的是上一页还是下一页首页最后一页（first,pre,next,last）
        apiVersion: 'v2', //  适配原来的v3接口，默认是v2，v3会有以下默认值，例如：分页参数路径化
        operateWidth: 240, //  操作列宽度
        operate: false, //  是否显示操作，是一个对象
        lang: _utils2.default.getLang(), // 语言配置  目前只支持en和cn
        ...
        var _picker = {
            option: {
                locale: _bs_table2.default[_utils2.default.getLang()]['daterangepicker'],
                autoUpdateInput: false
            },
            eventHandle: function eventHandle(ev, picker) {
                var $this = $(this),
        ...
        var _searcherDef = {
            ...
            numberange: {
                option: $.extend(true, {}, _bs_table2.default[_utils2.default.getLang()]['rangepicker']),
                event: 'okclick.range',
                editor: 'rangepicker'
            }
        };

        def = $.extend(true, {}, def, _bs_table2.default[_utils2.default.getLang()]['table']);
        ...
        function createTable($ele, options) {
            ...
            if (opts.showRowNumber) {
                opts.columns = [{
                    ...
                    align: 'center',
                    field: 'bs-rownumber',
                    formatter: function formatter(val, row, index) {
            ...
            if (opts.showToolbar) {
                ...
                var toolbar = [];
                toolbar.push('<div style="height:34px;line-height:30px;margin-bottom: 10px">', '<div class="pull-left col-xs-6">', '每页 <select style="width: 80px;" select2>');

                $.each(opts.pageList, function () {
                    toolbar.push('<option value="', this, '">', this, '</option>');
                });

                toolbar.push('</select> 条记录', '</div>', '<div class="pull-right col-xs-4">', '<div class="col-xs-4 text-right">搜索：</div>', '<div class="col-xs-8"><input type="text"></div>', '</div>', '</div>');
            ...
            // 显示操作列
            if (opts.operate && _typeof(opts.operate) === 'object') {
                ...
                operaColumn.formatter = function (val, row) {
                    ...
                    $.each(opts.operate, function (k, v) {
                        if ($.isFunction(v) && v !== $.noop) {
                            operaHtml.push('<button role="opera_', k, '" class="table_opera list-link" title="', opts.tips[k], '" >', '<i class="', opts.icons[k], '"></i>', '</button>');
                            ...
                            if (v.click && v.click != $.noop) {
                                operaHtml.push('<button role="opera_', k, '" class="table_opera  list-link" title="', opts.tips[k], '" ', enable ? '' : 'disabled="disabled"', '>', '<i class="', opts.icons[k], '"></i>', '</button>');
                            }
                        }
                    });
                    operaHtml.push('</div>');
            ...
            // 显示搜索头部信息
            if (opts.searchable) {
                var _initSearcher = function _initSearcher(table) {
                    // 查询支持
                    var $container = table.$tableContainer;
                    var $thead = $container.find('thead'); //  获取第一个表头
                    if ($thead.find('tr.header-search').length) {
                        //  已经渲染了搜索直接返回
                        return;
                    }
                    opts = table.options;
                    ...
                    $.each(opts.columns[0], function (i, cln) {
                        ...
                        // tr.push('<th>');
                        if (opts.singleSelect && !opts.showCheckBox && cln.checkbox) {
                            //  如果是单选并且不显示checkbox渲染空列
                                ...
                                if ($.isArray(cln.searcher.data)) {
                                    ...
                                    if ($.isArray(data)) {
                                        ...
                                        data.then(function (d) {
                                            ...
                                            $.each(d, function () {
                                                $(['<option value="', this[cln.searcher.valueField], '">', this[cln.searcher.textField], '</option>'].join('')).appendTo($select);
                                            });
                                        });
                                    }
                                }
                                tr.push('</select>');
                            } else if (cln.searcher.type == 'daterange' || cln.searcher.type == 'date' || cln.searcher.type == 'numberange') {
                                //  时间范围的处理
                     ...
                     * 表头搜索处理，自动查询
                     */
                    $container.undelegate('[data-field="bs-search"] i', 'click').delegate('[data-field="bs-search"] i', 'click', extraFn.showSearcher).undelegate('i.searcher-hide', 'click').delegate('i.searcher-hide', 'click', extraFn.hideSearcher).undelegate('.header-search select', 'change').delegate('.header-search select', 'change', function (e) {
                        var $target = $(e.target);
                        $ele.trigger('searcher-change.bs.table', [$target.attr('search-field'), $target.val()]);
                    }).undelegate('.header-search input', 'keyup').delegate('.header-search input', 'keyup', function (e) {
                        var $target = $(e.target);
                        $ele.trigger('searcher-change.bs.table', [$target.attr('search-field'), $target.val()]);
                    });
                };
            ...
            $ele.on('loading.bs.table', function (e, tableObj) {
                //  不显示无数据
                tableObj.$tableContainer.find('.no-records-found').find('div').css('visibility', 'hidden');
            }).on('loaded.bs.table', function (e, tableObj) {
                //  显示无数据
        ...
        function listenEvent($ele, scope, tId) {
            var events = ['all.bs.table', 'click-row.bs.table', 'dbl-click-row.bs.table', 'click-cell.bs.table', 'dbl-click-cell.bs.table', 'sort.bs.table', 'check.bs.table', 'uncheck.bs.table', 'check-all.bs.table', 'uncheck-all.bs.table', 'check-some.bs.table', 'uncheck-some.bs.table', 'load-success.bs.table', 'load-error.bs.table', 'column-switch.bs.table', 'column-search.bs.table', 'page-change.bs.table', 'search.bs.table', 'toggle.bs.table', 'pre-body.bs.table', 'post-body.bs.table', 'post-header.bs.table', 'expand-row.bs.table', 'collapse-row.bs.table', 'expanded-row.bs.table', 'collapsed-row.bs.table', 'searcher-change.bs.table', 'searcher-hide.bs.table', 'searcher-show.bs.table', 'refresh-options.bs.table', 'refresh.bs.table', 'search-local.bs.table', 'search-server.bs.table'];
        ...
        function bindEvent($ele, scope, tId) {
            var events = ['getOptions', 'getSelections', 'getAllSelections', 'getData', 'getRowByUniqueId', 'getRowsHidden', 'getHiddenColumns', 'getVisibleColumns', 'getScrollPosition', 'collapseAllRows', 'expandAllRows', 'collapseRow', 'expandRow', 'toggleView', 'togglePagination', 'nextPage', 'prevPage', 'selectPage', 'filterBy', 'scrollTo', 'hideColumn', 'showColumn', 'resetWidth', 'resetView', 'uncheckBy', 'checkBy', 'uncheck', 'check', 'uncheckAll', 'load', 'checkAll', 'hideLoading', 'showLoading', 'resetSearch', 'refresh', 'updateCell', 'mergeCells', 'hideRow', 'showRow', 'updateRow', 'insertRow', 'removeByUniqueId', 'removeAll', 'remove', 'prepend', 'refreshOptions', 'append', 'hideAllColumns', 'showAllColumns'];
