;(function ($) {
    var MODULE_NAME = "p_customer.customer_rz";
    var g_count = [6,24,7,30,12];
    function getRcText(sRcName)
    {
        return Utils.Base.getRcString("customer_rz_rc", sRcName).split(",");
    }
    function drawTerminaltype(sType)
    {
        ...
        $("#pTerminal").echart("init", option,oTheme);
    }
    function drawLoginUser(snr, newdata, date) {
        ...
        $("#portalUser").echart("init", option, oTheme);
    }
    ...
    function showEchart_time(data) {
        ...
        $("#onlinePie").echart("init", option, oTheme);
        var normalnumber = getRcText("USER_TYPE1111");
        var appendTohtml = [
            '<div style="position: relative;top: -160px;margin: 0 auto;width: 100px;"><div><span style="font-size: 16px; color:#343e4e; display: block;float: left; margin-left: 18px; margin-top: 32px;">',
            normalnumber[1],
            '</span>',
            '</div>'
        ].join(" ");
    ...
    function showEchart_access(datalist) {
       ...
       var option = {
        ...
        $("#ViewPie").echart("init", option, oTheme);
        var normalnumber = getRcText("USER_TYPE1111");
        var appendTohtml = [
            '<div style="position: relative;top: -160px;margin: 0 auto;width: 100px;"><div><span style="font-size: 16px; color:#343e4e; display: block;float: left; margin-left: 18px; margin-top: 32px;">',
            normalnumber[0],
    ...
    function drawTerminalfirm(legendOfOK,dataOfOK){
        var deviceChangDu=dataOfOK.length;
        if(deviceChangDu==0)
        {
            drawEmptyPie_rn($("#pTermianlFirm"));
            return;
        }
        var option = {
            height:210,
        ...
        var oTheme = {
            color : ['#fbceb1','#4ec1b2','#b3b7dd','#4fc4f6','#fe808b','#e7e7e9']
        };
        $("#pTermianlFirm").echart ("init", option,oTheme);
    }
    function getclientApGroup(){
        function getclientApGroupOK(data){
            if(!data.errcode)
            {
            var g_allInforbyAp=[];
        ...
        function getclientApGroupFail(err){
            console.log("ajax request fail:"+err);
        }
        var optGroup = {
            type:"GET",
            url:MyConfig.path+"/stamonitor/getclientlistbycondition?devSN="+FrameInfo.ACSN + "&reqType=group&auth=1",
            contentType:"application/json",
            dataType:"json",
            onSuccess:getclientApGroupOK,
            onFailed:getclientApGroupFail
        };
        Utils.Request.sendRequest(optGroup);
    }
    ...
    {
        Utils.Base.openDlg("p_customer.opendalg_rz", {"a":aType,"b":sName,"c":type}, {scope:$("#maoxian"),className:"modal-super dashboard"});
        return false;
    }
    ...
    function aj_getnewuser(mode,para) {
        ...
        var opt = {
            url:MyConfig.path+"/stamonitor/histclientstatistic_bycondition"+"?devSN=" + FrameInfo.ACSN + para + "&auth=1",
            type: "GET",
            dataType: "json",
            contentType: "application/json",
    ...
    function aj_getAccessTime(callback) {
        var ajax = {
            url:MyConfig.path+"/visitor/getstatisticsbycount?devSN=" + FrameInfo.ACSN+"&auth=1",
            type: "GET",
            dataType: "json",
            contentType: "application/json",
    ...
    function aj_getOnlineTime() {
        var ajax = {
            url:MyConfig.path+"/stamonitor/getclientlistbycondition?devSN=" + FrameInfo.ACSN + "&reqType=onlinetime&auth=1",
            type: "GET",
            dataType: "json",
            contentType: "application/json",
    ...
    function initEvent(){
        $("#portalClient a.time-link").click(function () {
            $("#portalClient a.time-link").removeClass("active");
            var val = $(this).addClass("active").attr("value");
            proUserCount(val);
        });
        $("#onlineCustomer").click(function(){
            Utils.Base.redirect({ np: "p_customer.clientinfo_rz"});
        });
        $("#historyCustomer").click(function(){
            Utils.Base.redirect({ np: "p_customer.clienthist_rz"});
        });
    }
    function initData(){
        initPieChart();
        function getclientstatisticbybandtypeOK(data){
            if(!data.errcode)
            {
                ...
                $("#portalNum5G").html(Num5GValue);
                $("#portalNum2G").html(Num2GValue);
                $("#portalTotal").html(TotalValue);
            }
        }
        ...
        var getclientstatisticbyband = {
            type:"GET",
            url:MyConfig.path+"/stamonitor/getclientlistbycondition?devSN="+FrameInfo.ACSN + "&reqType=count&auth=1" ,
            contentType:"application/json",
            dataType:"json",
        ...
        var getclientstatisticbyssid = {
            type:"GET",
            url:MyConfig.path+"/stamonitor/getclientlistbycondition?devSN="+FrameInfo.ACSN + "&reqType=ssid&auth=1",
            contentType:"application/json",
            dataType:"json",
        ...
        function getclientstatisticbymodeOK(data){
            if(!data.errcode)
            {
                drawTerminaltype(data.clientList);
            }
            else{
            }
        }
        ...
        var getclientstatisticbymode = {
            type:"GET",
            url:MyConfig.path+"/stamonitor/getclientlistbycondition?devSN="+FrameInfo.ACSN + "&reqType=mode&auth=1",
            contentType:"application/json",
            dataType:"json",
        ...
        function getclientstatisticbybyodOK(data){
            if(!data.errcode)
            {
                ...
                $.each(newObject,function(i,item){
                    legendOfOK[i]=item._id || getRcText ("VIEW_TITLE");
                    dataOfOK[i]={};
                    dataOfOK[i].name=item._id || getRcText ("VIEW_TITLE");
                    dataOfOK[i].value=item.clientCount || undefined;
                });
                drawTerminalfirm(legendOfOK,dataOfOK);
            }
            else{
            }
        }
        ...
        var getclientstatisticbybyod = {
            type:"GET",
            url:MyConfig.path+"/stamonitor/getclientlistbycondition?devSN="+FrameInfo.ACSN + "&reqType=vendor&auth=1",
            contentType:"application/json",
            dataType:"json",
