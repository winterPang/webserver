...
;(function ($) {
  var resizeableImage = function (image_target) {
    ...
    init = function () {
      ...
      $(".resize-image").remove();
      var img = $('<img class="resize-image" src="" alt="image for resizing" style="max-height: 1500px;max-width: 1500px">');
      img.insertAfter($(".overlay"));
      $('.resize-image').attr('src', image_target.src);
    ...
    crop = function () {
      ...
      var src=crop_canvas.toDataURL("image/jpeg", 0.8);
      src=src.split(',')[1];
      if(!src)return;
      src=window.atob(src);

      g_ia = new Uint8Array(src.length);
      for (var i = 0; i < src.length; i++) {
          g_ia[i] = src.charCodeAt(i);
      };
      //window.open(crop_canvas.toDataURL("image/jpeg", 0.8));
      $("#chatSettingBtnOk").removeClass("disabled");
      $("#chatSettingPortraintContent").hide();
      $("#chatSettingHeadImgOKimgContent").show();
      $("#chatSettingHeadImgOKimgContent img").attr("src",crop_canvas.toDataURL("image/jpeg", 0.8));
