;(function ($)
{
    var MODULE_NAME = "versionupdate.index";
    ...
    function downloadVersion(rowData)
    {
        ...
        if(rowData.length == 1){
            var data = {
                ...
                "dev_version_url":rowData[0].url
            }
            $.ajax({
                url: "/v3/ace/oasis/oasis-rest-shop/restshop/o2oportal/upgradeDevice",
                type: "POST",
                dataType: "json",
    ...
    function initData(){
        $.ajax({
            ...
            success: function(data){
                ...
                $.ajax({
                    url: "/v3/ace/oasis/oasis-rest-dev-version/restdev/o2oportal/getModelVersion?model="+model1,
                    type: "GET",
                    dataType: "json",
                    contentType: "application/json",
                    success: function(data) {
    ...
    function initFenJiFenQuan()
    {
        ...
        if($.inArray("EXEC",arrayShuZuNew)==-1){
            //隐藏“执行”的功能
            //执行
            ($(".slist-button[title='离线下载']")) .css('display','none');
        }

        if($.inArray("WRITE",arrayShuZuNew)==-1){
            //隐藏“写”的功能
            //写
            ($(".slist-button[title='升级版本']")) .css('display','none');
        }
    }
    function _destroy()
    {
