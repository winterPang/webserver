(function($) {
    ...
    function onDelete(aData){
        //取消发布-删除发布-解绑所有AP/APGroup-删除
        disPublish(aData[0].stName);
        deletePubmng(aData[0].stName);
        function ssidDeleteSuc(aData2){
            if(aData2.result=="fail"){
                Frame.Msg.info("删除失败,请检查设备","error");               
            }else if(aData2.result=="success"){
                if(aData2.deviceResult[0].result=="success"){
                    Frame.Msg.info("删除成功");
                    Utils.Base.refreshCurPage(); 
                }else if(aData2.deviceResult[0].result=="fail"){
    ...
    function addThemeTemplate(aData){
        if($("#btn1").prop("checked")){
            function addThemeTemplateSuc(data){ 
    ...
    function publish(){
        function publishSuc(data){  
            if(data.errorcode=="1308"){
               Frame.Msg.info("添加失败,同一场景下不能拥有相同的SSID","error"); 
            }                          
        }
