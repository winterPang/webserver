define(['css!../css/editmap',
    ...
    return ['$scope','$state','$stateParams','$alertService','$timeout',
                'locConst', 'locSer','jtopoSer',function (
                    $scope,$state,$stateParams,$alert,$timeout,
                        locConst,locSer,jtopoSer) {
            ...
            okHandler: function () {
                ...
                $.each(table.rows('.selected').data(), function (index, row) {
                    jtopoSer.addAp(scene, iX, iY, row.apName, row.apSn,{
                        fontColor:'176,23,31',
                        image:'../locationmanage/img/ap.png'
                    },nodeListener);
